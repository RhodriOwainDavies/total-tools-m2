define(
    [
        ...
        'mage/template',
        'jquery/ui',
        'Temando_Temando/js/checkout/storepickupcheckout'
    ],
    function ($, quote, url, $t, mageTemplate, newMap) {
        ...
        {
            var $wrapperelectHtml = $('<div class="list-store-to-pickup"><label>' +
                $t('Select Store:') + '</label></div>');
            var $selectStoreHtml = $('<select class="list-store-select disable-ul"></select>');
            $selectStoreHtml.append('<option class="show-tag-li store-item" value="">' +
                $t('Select a store to pickup') + '</option>');
            $.each(liststoreJson, function (index, el) {
                var stock_level_message = el.stock_level_message;
                if (stock_level_message != '') {
                    stock_level_message = ' (' + stock_level_message + ')';
                }
                $selectStoreHtml.append('<option class="show-tag-li store-item" value="' +
                    el.origin_id + '">' + el.name + stock_level_message + '</option>');
            });
        ...
        quote.shippingMethod.subscribe(function (value) {
            ...
            if ((!$('.storepickup-information').length > 0)) {
            ...
            if (quote.shippingMethod().carrier_code == 'storepickup') {
                if (!($('.info-store-checkout').length>0) ||
                    ...
                    ((isDisplayPickuptime) && (($('#shipping_date').val() == '') ||
                    ($('#shipping_time').val() == '-1')))) {
                ...
                if (!($('.list-store-to-pickup').length > 0)) {
                    ...
                    $('.list-store-select').change(function () {
                        ...
                        $('.overlay-bg-checkout').show();
                        $.each(liststoreJson, function (index, el) {
                            if (el.origin_id == $('.list-store-select').val()) {
                                storePikcuplatitude= el.latitude;
                                storePikcuplongitude=el.longitude;
                                var store_information = '<h3>' + el.name + '</h3><br/>' +
                                    '<p>' + $t('Store address: ') + el.street + '</p>'+
                                    '<p>' + $t('Store Phone: ') + el.contact_phone_1 + '</p>';
                                if ($('.info-store-checkout').length > 0) {
                                    $('.info-store-checkout').html(store_information);
                                    $('.info-store-checkout').show();
                                } else {
                                    var info_store = '<div class ="info-store-checkout">' +
                                        '<h3>'+ el.name + '</h3><br/>' +
                                        '<p>' + $t('Store address: ') + el.street +
                                        '</p>'+'<p>' + $t('Store Phone: ') + el.contact_phone_1 +
                                        '</p>' + '</div>';
                                    $('#select_store_by_map').after(info_store);
                                }
                                $.ajax(
                                    {
                                        url: url.build("temando/checkout/changestore"),
                                        type: "post",
                                        dateType: "text",
                                        data: {
                                            origin_id: $('.list-store-select').val()
                                        },
                ...
                $.each(liststoreJson, function (index, el) {
                    if (el.origin_id == defaultStore && !($('.info-store-checkout').length>0 )) {
        ...
        {
            $.ajax({
                url: url.build("temando/checkout/disabledate"),
                type: "post",
                dateType: "json",
                data: {
                    origin_id: $('.list-store-select').val()
        ...
        function showTimeBox(shipping_date_val, store_id_val)
        {
            if (!($('#shipping_time_div').length > 0)) {
                ...
                $("#shipping_time").change(function () {
                    if (($("#shipping_time").val()!='-1')&&($('#shipping_date_div').length > 0)) {
                        $('.overlay-bg-checkout').show();
                        $.ajax({
                            url: url.build("temando/checkout/changetime"),
                            type: "post",
                            dateType: "json",
                            data: {
                                origin_id: $('.list-store-select').val(),
            ...
            $.ajax({
                url: url.build("temando/checkout/changedate"),
                type: "post",
                dateType: "json",
                data: {
                    shipping_date: shipping_date_val,
                    origin_id:store_id_val
                },
                success: function (result) {
                    result = $.parseJSON(result);
                    $('#shipping_time').html("");
